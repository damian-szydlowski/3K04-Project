...
\begin{enumerate}[label=]
    ...
    \begin{tcolorbox}
        \begin{figure}[H]
            \centering 
            \includegraphics[width=\textwidth]{echover.png}
            \caption{Echo Appearing in DCM}
        \end{figure}
    \end{tcolorbox}
\end{enumerate}
...
\subsubsection{Push Button Shutoff}
\begin{enumerate}[label=]
    \item \textbf{Safety Goal:} A quick way to manual shutoff pacing of ventricular pulses.
    \item \textbf{Rationale:} In case the ventricular pacing malfunctions a pushbutton to quickly stop the pacing can be used. 
    \item \textbf{Evidence:} Shown in demo but can be seen in the code logic
    \begin{tcolorbox}
        \begin{figure}[H]
            \centering 
            \includegraphics[width=\textwidth]{ventinhib.png}
            \caption{Simulink Inhibiting Ventricular Pulses}
        \end{figure}
    \end{tcolorbox}
\end{enumerate}
...
\begin{enumerate}[label=]
    ...
    \item \textbf{Strict Pacing Times:} The pacing loop must execute within a strict maximum execution time. If a pulse needs 
    to be accurate the code can not be taking time to process serial data. 
    \item \textbf{Must be Optimized for Battery Life:} Our pacemaker has nearly infinite power being connected to dedicated power supplies, however, pacemakers need 
    to run off batteries for very long periods of time. 
    \item \textbf{Magnet Safe:} The pacemaker may experience EMI and will need to be able to pass a magnet test before being able to 
    be deployed. EMI may cause malfunctions or failures of equipment and is thus crucial for safety cricital operation.
...
\begin{enumerate}[label=]
    \item \textbf{Interupt Based Approach:} Using hardware based timers to detect events rather than polling.
    \item \textbf{Implementation of a Watchdog:} Using a watch dog timer to detect when a failure occurs such as a looping or no pulse when one is needed and fixing accordingly. 
    A doctor can not be watching a patient 24/7 and must be able to correct its own errors. 
\end{enumerate}

\newpage
\section{Module Description}
\subsection{Simulink System (Model-Based Design)}

\subsubsection*{Description}
The Simulink project functions as a closed-loop simulation environment designed to validate the pacemaker logic against a simulated patient heart model before hardware deployment. It consists of two primary subsystems: the \textbf{Device Model} (Pacemaker) and the \textbf{Environment Model} (Heart \& Activity).

\subsubsection*{Key Sub-Modules}

\subsubsubsection*{1. Heart \& Patient Model (Environment)}
\begin{itemize}
    \item \textbf{Function:} Simulates intrinsic cardiac activity (Atrial and Ventricular signals) and patient physical activity.
    \item \textbf{Inputs:} User-adjustable sliders for Natural Heart Rate (bpm), pulse widths, and AV Delay.
    \item \textbf{Outputs:} Generates analog-like electrical signals representing the heart's intrinsic behavior to feed into the sensing channels of the pacemaker.
\end{itemize}

\subsubsubsection*{2. Pacemaker Logic (Device)}
\begin{itemize}
    \item \textbf{State Machine:} Implements the timing cycles and logic for standard pacing modes (AOO, VOO, AAI, VVI) and their rate-adaptive counterparts (AOOR, VOOR, AAIR, VVIR) as well as the bonus DDDR.
    \item \textbf{Rate Adaptation Block:} Processes simulated accelerometer input to calculate the activity level. It utilizes tunable parameters found in the testing documents: Activity Threshold, Reaction Time (time to increase rate), Response Factor, and Recovery Time (time to decrease rate).
    \item \textbf{Timing Control:} Manages Lower Rate Limit (LRL), Upper Rate Limit (MSR), Refractory Periods (ARP/VRP/PVARP), and Hysteresis.
    \item \textbf{Pulse Generation:} Outputs logical pacing triggers based on the expiration of timing cycles or lack of intrinsic sensing.
\end{itemize}

\subsection{DCM Application}

\subsubsection*{Description}
The DCM is a desktop application built using Python and \texttt{customtkinter} that follows a Model-View-Controller (MVC) architecture. It manages user authentication, programmable device parameters, real-time data visualization, and serial communication with the pacemaker hardware (FRDM-K64F).

\subsubsection*{Architecture Breakdown}

\subsubsubsection*{A. Controller (\texttt{controller.py})}
\begin{itemize}
    \item \textbf{Role:} The core of the application.
    \item \textbf{Functionality:}
    \begin{itemize}
        \item Initializes the main application window and manages navigation between views (Welcome, Login, Main Dashboard, Egrams).
        \item Holds the global state for the active session (current user, serial connection status, font size settings).
        \item Mediates data flow between the GUI Views and the Backend Models.
        \item Manages the background thread for Text-to-Speech audio feedback upon connection.
    \end{itemize}
\end{itemize}

\subsubsubsection*{B. Models (Data \& Logic Layers)}
\begin{itemize}
    \item \textbf{User Model (\texttt{models/user\_model.py}):}
    \begin{itemize}
        \item Manages the local database (\texttt{users.json}).
        \item Handles user registration (limit 10 users) and authentication logic.
    \end{itemize}
    \item \textbf{Pacing Model (\texttt{models/pacing\_model.py}):}
    \begin{itemize}
        \item Manages persistence of device parameters (\texttt{pacing\_settings.json}).
        \item Stores and retrieves parameter sets keyed by \texttt{User $\to$ Pacing Mode}.
    \end{itemize}
    \item \textbf{Serial Communication (\texttt{models/serial\_comms.py}):}
    \begin{itemize}
        \item \textbf{Driver Layer:} Wrapper around \texttt{pyserial}. Handles port scanning, connection, and disconnection.
        \item \textbf{Protocol Handling:} Implements the byte-level communication protocol using \texttt{struct}.
        \begin{itemize}
            \item \textit{Parameter Send:} Packs 18 parameters (Mode, LRL, MSR, Amplitudes, Pulse Widths, Rate Adaptive settings) into binary packets.
            \item \textit{Echo/Verification:} Requests and unpacks data from the hardware to verify saved settings.
            \item \textit{Egram Streaming:} Implements a sync-based reader that buffers incoming real-time cardiac data (Atrial/Ventricular floats) for graphing.
        \end{itemize}
    \end{itemize}
\end{itemize}

\subsubsubsection*{C. Views (GUI Layer)}
\begin{itemize}
    \item \textbf{Login Views (\texttt{views/login\_views.py}):}
    \begin{itemize}
        \item Provides the ``Welcome'' and ``Register'' screens.
        \item Handles input validation for creating new users and logging in.
    \end{itemize}
    \item \textbf{Main View (\texttt{views/main\_view.py}):}
    \begin{itemize}
        \item \textbf{Dashboard:} The primary interface for the user.
        \item \textbf{Mode Selection:} Allows selection of 8 pacing modes (AOO, VVI, VVIR, etc.).
        \item \textbf{Parameter Entry:} Dynamically generates input fields based on the selected mode. Includes strict input validation (e.g., LRL 30-175ppm, Reaction Time 10-50s) and error handling.
        \item \textbf{Hardware Debug:} A dedicated panel for testing LED indicators on the board via serial commands.
    \end{itemize}
    \item \textbf{Egram View (\texttt{views/egram\_view.py}):}
    \begin{itemize}
        \item \textbf{Real-Time Plotting:} Uses \texttt{matplotlib} with the \texttt{TkAgg} backend to render electrograms.
        \item \textbf{Performance:} Implements Blitting (drawing only changed pixels) to ensure smooth animation of high-frequency signal data.
        \item \textbf{Controls:} Allows the user to Start/Stop streaming and toggle between Atrial, Ventricular, or Dual-channel views.
    \end{itemize}
\end{itemize}

\subsubsubsection*{D. Accessibility Features}
\begin{itemize}
    \item \textbf{Global Font Scaling:} The application supports dynamic font resizing (A+/A- buttons) across all views to assist visually impaired users.
    \item \textbf{Audio Feedback:} Uses \texttt{pyttsx3} to provide auditory confirmation when the pacemaker hardware is successfully connected.
\end{itemize}
...
\begin{tcolorbox}
    \begin{figure}[H]        
        \label{AOOtest}
        \centering
        \includegraphics[width=0.9\textwidth]{AOOtest1.png}
        \caption{AOO Test}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \label{AOOpulse}
        \centering
        \includegraphics[width=0.9\textwidth]{AOOPulseClose.png}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \label{VOOtest}
        \centering
        \includegraphics[width=0.9\textwidth]{VOOtest.png}
        \caption{VOO Test}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \label{VOOpulseclose}
        \centering
        \includegraphics[width=0.9\textwidth]{VOOpulseclose.png}
...
\newpage
\subsection{Testing of AOOR}

\subsubsection{Test 1: AOOR at Rest (No Movement)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AOOR functionality at rest.
    \item \textbf{Input Conditions:} General Inputs mode = 4 (AOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs.
    \item \textbf{Expected Output:} Consistent, evenly spaced pulses in the output with disregard for natural heartbeats in the atrium signal.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}

\begin{tcolorbox}
    \begin{figure}[H]
        \includegraphics[width=\textwidth]{aoort1.png}
        \caption{AOOR at Rest Ignoring Pulses}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \includegraphics[width=\textwidth]{aoort1.png}
        \caption{AOOR at Resting Pace Rate}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 2: AOOR with Increased Movement}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AOOR functionality while the patient has increased movement.
    \item \textbf{Input Conditions:} General Inputs mode = 4 (AOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Also the pacemaker was shook at a medium rate for 5 seconds.
    \item \textbf{Expected Output:} Consistent, evenly spaced pulses in the output with disregard for natural heartbeats, at a faster rate than resting in the atrium signal.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \includegraphics[width=\textwidth]{aoort3.png}
        \caption{AOOR at Increased Pace Rate from Movement}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 3: AOOR Maximum Sensor Rate}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AOOR functionality while the patient has increased movement for a prolonged period of time.
    \item \textbf{Input Conditions:} General Inputs mode = 4 (AOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Also the pacemaker was shook vigorously for 30 seconds.
    \item \textbf{Expected Output:} Consistent, evenly spaced pulses in the output with disregard for natural heartbeats, at the capped maximum sensor rate in the atrium signal.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{aoort4.png}
        \caption{AOOR at Maximum Pace Rate}
    \end{figure}
\end{tcolorbox}

\newpage
\subsection{Testing of VOOR}

\subsubsection{Test 1: VOOR at Rest (No Movement)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VOOR functionality.
    \item \textbf{Input Conditions:} General Inputs mode = 5 (VOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs.
    \item \textbf{Expected Output:} Consistent, evenly spaced pulses in the output with disregard for natural heartbeats in the ventricular signal.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{voort1.png}
        \caption{VOOR at Rest and No Natural Heart Rate}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{voort2.png}
        \caption{VOOR at Rest with a Natural Heart Rate}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 2: VOOR with Increased Movement}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VOOR functionality while the patient has increased movement.
    \item \textbf{Input Conditions:} General Inputs mode = 5 (VOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Also the pacemaker was shook at a medium rate for 5 seconds.
    \item \textbf{Expected Output:} Consistent, evenly spaced pulses in the output with disregard for natural heartbeats in the ventricular signal, at a higher pace then resting.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{voort3.png}
        \caption{VOOR with an Increased Heart Rate from Movement}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 3: VOOR Maximum Sensor Rate}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VOOR functionality while the patient has increased movement for prolonged period of time.
    \item \textbf{Input Conditions:} General Inputs mode = 5 (VOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Also the pacemaker was shook vigorously for 30 seconds.
    \item \textbf{Expected Output:} Consistent, evenly spaced pulses in the output with disregard for natural heartbeats, at the capped maximum sensor rate in the ventricular signal.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{voort4.png}
        \caption{VOOR at Maximum Sensor Rate}
    \end{figure}
\end{tcolorbox}

\newpage
\subsection{Testing of AAIR}

\subsubsection{Test 1: AAIR at Rest (0 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AAIR functionality at rest with no natural heart beat of 0 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 6 (AAIR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Natural heart rate was set to 0 bpm.
    \item \textbf{Expected Output:} Consistent, evenly spaced pulses in the output with disregard for natural heartbeats in the atrium signal.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{aairt1.png}
        \caption{AAIR with no Natural Heart Rate at Rest}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 2: AAIR at Rest (45 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AAIR functionality at rest with natural heart beat at 45 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 6 (AAIR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Natural heart rate set to 45 bpm.
    \item \textbf{Expected Output:} The pacemaker should pulse after the refractory period expires. An output of blue pulses right before the natural red pulses is expected.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{aairt2.png}
        \caption{AAIR with a Natural Heart Rate at 45BPM at Rest}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 3: AAIR at Rest (75 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AAIR functionality at rest with natural heartbeat of 75 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 6 (AAIR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Natural heart rate was set to 75 BPM.
    \item \textbf{Expected Output:} As a nominal heart rate is being inputted, the pacemaker should not be delivering pacing pulses as the simulated heart rate is nominal and healthy.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{aairt3.png}
        \caption{AAIR with a Natural Heart Rate at 75BPM at Rest}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 4: AAIR with Increased Movement (45 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AAIR functionality at increased movement with natural heartbeat of 45 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 6 (AAIR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Natural heart rate was set to 45 BPM. Also the pacemaker was shaken at a medium rate for 5 seconds.
    \item \textbf{Expected Output:} Since the activity level increases, the blue pulse paces need to occur faster to keep up with the increased movement. The blue pulses should have an increased bpm, while still reacting to the natural heart, pulsing when refractory period expires, in the atrium.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{aairt4.png}
        \caption{AAIR with a Natural Heart Rate at 45BPM at with Movement}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 5: AAIR Maximum Sensor Rate (45 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AAIR functionality at increased movement over a prolonged period with natural heartbeat of 45 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 6 (AAIR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Natural heart rate was set to 45 BPM. Also the pacemaker was shook vigorously for 30 seconds.
    \item \textbf{Expected Output:} The pacemaker should be pacing at the max sensor rate, of 120bpm. An output of 2 blue pulses right before the natural red pulses is expected, since the bpm is double that of resting, in the atrium.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{aairt5.png}
        \caption{AAIR with a Natural Heart Rate at 47BPM at Max Sensor Rate}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 6: AAIR Maximum Sensor Rate (75 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AAIR functionality at increased movement over a prolonged period with natural heartbeat of 75 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 6 (AAIR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Natural heart rate was set to 75 BPM. Also the pacemaker was shook vigorously for 30 seconds.
    \item \textbf{Expected Output:} The pacemaker should pulse after the refractory period expires. An output of blue pulses right before the natural red pulses is expected in the atrium.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{aairt6.png}
        \caption{AAIR with a Natural Heart Rate at 75BPM at Max Sensor Rate}
    \end{figure}
\end{tcolorbox}

\newpage
\subsection{Testing of VVIR}

\subsubsection{Test 1: VVIR at Rest (0 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VVIR functionality at rest with no natural heartbeat of 0 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 7 (VVIR), hysteresis = 0 (off), standard ventricular inputs as well as standard rate adaptive inputs. Natural heart rate was set to 0 bpm.
    \item \textbf{Expected Output:} Consistent, evenly spaced pulses in the output with disregard for natural heartbeats in the ventricular signal.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{vvirt1.png}
        \caption{VVIR with no Nratural Heart Rate at Rest}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 2: VVIR at Rest (45 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VVIR functionality at rest with natural heart beat at 45 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 7 (VVIR), hysteresis = 0 (off), standard ventricular inputs as well as standard rate adaptive inputs. Natural heart rate set to 45 bpm.
    \item \textbf{Expected Output:} The pacemaker should pulse after the refractory period expires. An output of blue pulses right before the natural red pulses is expected in the ventricular.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{vvirt2.png}
        \caption{VVIR with a Nratural Heart Rate of 45BPM at Rest}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 3: VVIR at Rest (75 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VVIR functionality at rest with natural heartbeat of 75 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 7 (VVIR), hysteresis = 0 (off), standard ventricular inputs as well as standard rate adaptive inputs. Natural heart rate was set to 75 BPM.
    \item \textbf{Expected Output:} As a nominal heart rate is being inputted, the pacemaker should not be delivering pacing pulses as the simulated heart rate is nominal and healthy.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{vvirtt3.png}
        \caption{VVIR with a Nratural Heart Rate of 75BPM at Rest}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 4: VVIR with Increased Movement (45 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VVIR functionality at increased movement with natural heartbeat of 45 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 7 (VVIR), hysteresis = 0 (off), standard ventricular inputs as well as standard rate adaptive inputs. Natural heart rate was set to 45 BPM. Also the pacemaker was shaken at a medium rate for 5 seconds.
    \item \textbf{Expected Output:} Since the activity level increases, the blue pulse paces need to occur faster to keep up with the increased movement. The blue pulses should have an increased bpm, while still reacting to the natural heart, pulsing when refractory period expires, in the ventricular.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{vvirt4.png}
        \caption{VVIR with a Nratural Heart Rate of 45BPM with Movement}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 5: VVIR Maximum Sensor Rate (45 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VVIR functionality at increased movement over a prolonged period with natural heartbeat of 45 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 7 (VVIR), hysteresis = 0 (off), standard ventricular inputs as well as standard rate adaptive inputs. Natural heart rate was set to 45 BPM. Also the pacemaker was shook vigorously for 30 seconds.
    \item \textbf{Expected Output:} The pacemaker should be pacing at the max sensor rate, of 120bpm. An output of 2 blue pulses right before the natural red pulses is expected, since the bpm is double that of resting, in the ventricular.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{vvirt5.png}
        \caption{VVIR with a Nratural Heart Rate of 45BPM with Max Sensor Rate}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 6: VVIR Maximum Sensor Rate (75 BPM)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic VVIR functionality at increased movement over a prolonged period with natural heartbeat of 75 bpm.
    \item \textbf{Input Conditions:} General Inputs mode = 7 (VVIR), hysteresis = 0 (off), standard ventricular inputs as well as standard rate adaptive inputs. Natural heart rate was set to 75 BPM. Also the pacemaker was shook vigorously for 30 seconds.
    \item \textbf{Expected Output:} The pacemaker should pulse after the refractory period expires. An output of blue pulses right before the natural red pulses is expected in the ventricular.
    \item \textbf{Actual Output:} Since the refractory period is longer for ventricular by default (320 instead of 250) the refractory period expires right before the natural pulse on occasion, which is why we see the pattern. This is expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{vvirt6.png}
        \caption{VVIR with a Nratural Heart Rate of 75BPM with Max Sensor Rate}
    \end{figure}
\end{tcolorbox}

\newpage
\subsection{Rate Adaptive Tuning Testing}

\subsubsection{Test 1: Recovery Time Testing (30 seconds)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AOOR functionality for the recovery time tuning.
    \item \textbf{Input Conditions:} General Inputs mode = 4 (AOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Also the pacemaker was shook at a medium rate for 30 seconds. Recovery time was set to 0.5 (translates to 30 seconds).
    \item \textbf{Expected Output:} After reaching the upper rate limit, it should take 30 seconds for the pacemaker to return to the lower rate limit.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{rec1.png}
        \caption{AAIR at Time 0s After Movement}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{rec2.png}
        \caption{AAIR at Time 30s After Movement}
    \end{figure}
\end{tcolorbox}

\subsubsection{Test 2: Recovery Time Testing (60 seconds)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AOOR functionality for the recovery time tuning.
    \item \textbf{Input Conditions:} General Inputs mode = 4 (AOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Also the pacemaker was shook at a medium rate for 30 seconds. Recovery time was set to 1 (translates to 60 seconds).
    \item \textbf{Expected Output:} After reaching the upper rate limit, it should take 60 seconds for the pacemaker to return to the lower rate limit.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{rec11.png}
        \caption{AAIR at Time 0s After Movement}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{rec12.png}
        \caption{AAIR at Time 60s After Movement}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 3: Reaction Time Testing (10 seconds)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AOOR functionality for the reaction time tuning.
    \item \textbf{Input Conditions:} General Inputs mode = 4 (AOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Also the pacemaker was shook at a medium rate until the maximum sensor rate. Recovery time was set to 10 seconds.
    \item \textbf{Expected Output:} The pacemaker should reach the upper rate limit in 10 seconds.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        ...
        \caption{AAIR at Time 0s (Before Movement)}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        ...
        \caption{AAIR After 10s of Shaking}
    \end{figure}
\end{tcolorbox}

\newpage
\subsubsection{Test 4: Reaction Time Testing (30 seconds)}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} The purpose of this test is to test basic AOOR functionality for the reaction time tuning.
    \item \textbf{Input Conditions:} General Inputs mode = 4 (AOOR), hysteresis = 0 (off), standard atrial inputs as well as standard rate adaptive inputs. Also the pacemaker was shook at a medium rate until the maximum sensor rate. Recovery time was set to 30 seconds.
    \item \textbf{Expected Output:} The pacemaker should reach the upper rate limit in 30 seconds.
    \item \textbf{Actual Output:} Output tested exactly as expected.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{rect12.png}
        \caption{AAIR at Time 0s (Before Movement)}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{rect22.png}
        \caption{AAIR After 30s of Shaking}
    \end{figure}
\end{tcolorbox}
...
\subsubsection{Font Size Accessibility Controls}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} To verify that font size controls improve accessibility by allowing users to enlarge or reduce text for clearer visibility.
    \item \textbf{Input Conditions:} The user presses either \textbf{A+} to increase font size or \textbf{A-} to decrease font size.
    \item \textbf{Expected Output:} All visible text elements update to a larger or smaller size immediately, including labels, buttons, radio buttons, and plot titles. The layout remains stable.
    \item \textbf{Actual Output:} Pressing \textbf{A+} increased font size across all interface elements. Pressing \textbf{A-} reduced size consistently. All components updated correctly and remained properly aligned.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{fontsize.png}
        \caption{Accessibility Interface: Font Size Adjustment}
    \end{figure}
\end{tcolorbox}

\subsubsection{Manual Entry of ``OFF'' in Programmable Field}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} To confirm that typing ``OFF'' into a programmable parameter field is recognized as a valid symbolic value and updates the internal model correctly.
    \item \textbf{Input Conditions:} The user manually types ``OFF'' into a programmable text entry field, such as amplitude, sensitivity, or refractory period.
    \item \textbf{Expected Output:} The system should accept ``OFF'' as a valid entry, convert it to the correct internal value (such as zero or the OFF code), and display ``OFF'' in the interface without errors.
    \item \textbf{Actual Output:} Typing ``OFF'' was accepted. The field saved the OFF state, the internal parameter was updated correctly, and the value was transmitted in the outgoing packet with the proper OFF code.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{offtest.png}
        \caption{Allowing of OFF In Parameters}
    \end{figure}
\end{tcolorbox}

\subsubsection{Error When Incrementing Values Not by 0.1}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} To confirm that the system rejects values that are not valid increments of 0.1 and provides a clear error message.
    \item \textbf{Input Conditions:} The user types a value such as 2.37 or 0.15 into a field that only allows increments of 0.1.
    \item \textbf{Expected Output:} The system should detect the invalid step size, prevent saving the value, highlight the field, and show an error message explaining that only 0.1 increments are allowed.
    \item \textbf{Actual Output:} The system displayed an error, rejected the invalid value, and restored the previous valid value. No incorrect data was saved or transmitted.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{incre.png}
        \caption{Incorrect Incrementing of Value Test}
    \end{figure}
\end{tcolorbox}

\subsubsection{Amplitude Range 0.1--5.0 V}
\begin{enumerate}[label=]
    \item \textbf{Purpose:} Verify that the A or V Pulse Amplitude field rejects values outside the allowed range of 0.1 V to 5.0 V.
    \item \textbf{Input Conditions:} User types the following values into the amplitude field and presses Save:
    \begin{itemize}
        \item Test A: 5.5
        \item Test B: 0
        \item Test C: -0.1
    \end{itemize}
    \item \textbf{Expected Output:} For each invalid value the system should:
    \begin{itemize}
        \item Show an error message stating that amplitude must be between 0.1 V and 5.0 V.
        \item Prevent the value from being saved.
        \item Keep or restore the previous valid value.
    \end{itemize}
    \item \textbf{Actual Output:} For 5.5, 0, and -0.1, the DCM displayed an error message that the amplitude must be within 0.1--5.0 V, did not save the invalid entry, and left the previous valid amplitude in the field. No incorrect data was written to \texttt{pacing\_settings.json}.
    \item \textbf{Result:} Pass
\end{enumerate}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{high.png}
        \caption{Input Validation Error Dialogue (Too high)}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{low.png}
        \caption{Input Validation Error Dialogue (Too low)}
    \end{figure}
\end{tcolorbox}
...
\begin{tcolorbox}
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{neg.png}
        \caption{Input Validation Error Dialogue (Negative Values)}
    \end{figure}
\end{tcolorbox}
...
before inclusion. Furthermore, AI was to used as a search engine for debugging of both Simulink and DCM issues.
